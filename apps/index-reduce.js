document.addEventListener("DOMContentLoaded", function() {
  ...
  function iterator (articleData, start, end) {
    ...
    const itemDetailPromise = (itemID) => {
      return new Promise(function (resolve, reject) {
        $.get({
    ...
    list.reduce((previousPromise, itemID) => {
      return previousPromise.then(function () {
        return itemDetailPromise(itemID).then((item) => {
  ...
  function calcEnd () {
    if (save.length - count.start < 30) {
      count.end = save.length;
      isDone = !isDone;
  ...
  function getItems (i) {
    ...
    articleDetailPromise(i)
      .then(articleData => {
        save = articleData;
        console.log(`length: ${save.length}`);

        iterator(save, count.start, calcEnd());
      });
  }
  ...
  $more.addEventListener('click', function () {
    ...
    console.log(count.times, count.start, calcEnd());

    // getMore 반복해야 함 데이터가 없을 때까지
    iterator(save, count.start, count.end);
  });
